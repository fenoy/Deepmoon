...
for epoch in range(n_epochs):
    ...
    for i, data in enumerate(loader['train']):
        ...
        real_lossD = criterion(out_real, valid)
        real_lossD.backward()

        z = torch.randn(x_real.size(0), latent_size, 1, 1, device=device)
        x_fake = netG(z)
        out_fake = torch.sigmoid(netD(x_fake.detach()))
        fake_lossD = criterion(out_fake, fake)
        fake_lossD.backward()
        optimizerD.step()
        ...
        if i % n_print == 0:
            print(
                "[Epoch {:5}/{:5}] [Batch {:3}/{:3}] [D loss: {:2.6f}] [G loss: {:2.6f}]".format(
                    epoch, n_epochs, i, len(loader['train']), fake_lossD.item() + real_lossD.item(), lossG.item()
                )
            )
    ...
    for data in loader['val']:
        ...
        valFake_lossD = criterion(out_fake, fake)
        valLossD += (valReal_lossD + valFake_lossD) * 0.5
    ...
    history['lossD'] += [fake_lossD.item() + real_lossD.item()]
...
#with open("../final_results/" + str(model) + "_BCE" +"_seed" + str(seed) + ".csv", "w") as history_file:
#    json.dump(
